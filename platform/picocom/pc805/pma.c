...
void mcall_set_pma(unsigned long pa, unsigned long va, unsigned long size, unsigned long entry_id)
{
	...
	if (entry_id < PMA_NUM) {
		...
		*pmaxcfg = *pmaxcfg | PMA_NOCACHE_NOBUFFER;
...
inline unsigned long read_pmaaddr(int i)
{
	...
	if (i == 0)
		val = csr_read(PMAADDR_0);
	else if (i == 1)
		val = csr_read(PMAADDR_1);
	else if (i == 2)
		val = csr_read(PMAADDR_2);
	else if (i == 3)
		val = csr_read(PMAADDR_3);
	else if (i == 4)
		val = csr_read(PMAADDR_4);
	else if (i == 5)
		val = csr_read(PMAADDR_5);
	else if (i == 6)
		val = csr_read(PMAADDR_6);
	else if (i == 7)
		val = csr_read(PMAADDR_7);
	else if (i == 8)
		val = csr_read(PMAADDR_8);
	else if (i == 9)
		val = csr_read(PMAADDR_9);
	else if (i == 10)
		val = csr_read(PMAADDR_10);
	else if (i == 11)
		val = csr_read(PMAADDR_11);
	else if (i == 12)
		val = csr_read(PMAADDR_12);
	else if (i == 13)
		val = csr_read(PMAADDR_13);
	else if (i == 14)
		val = csr_read(PMAADDR_14);
	else if (i == 15)
		val = csr_read(PMAADDR_15);
